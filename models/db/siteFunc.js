...
var siteFunc = {
  ...
  getCategoryList: function() {
    return ContentCategory.find({
      'parentID': '0',
      'state': '1'
    }, 'name defaultUrl').sort({
      'sortId': 1
    }).find();
  },

  getHotItemListData: function(q) {
    return Content.find(q, 'stitle').sort({
      'clickNum': -1
    }).skip(0).limit(10);
  },
  ...
  getMessageList: function(contentId) {
    return Message.find({
      'contentId': contentId
    }).populate('author').populate('replyAuthor').populate('adminAuthor').exec();
  },
  ...
  setDataForCate: function(req, res, params, staticforder, defaultTempPath) {
    ...
    var currentCateList = ContentCategory.find(params.cateQuery).sort({
      'sortId': 1
    });
    var tagsData = DbOpt.getDatasByParam(ContentTags, req, res, {});
    return {
  ...
  setDataForUserReply: function(req, res, params, staticforder, defaultTempPath) {
    req.query.limit = 5;
    var documentList = DbOpt.getPaginationResult(Message, req, res, {
